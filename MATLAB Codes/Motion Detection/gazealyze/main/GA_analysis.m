        ...
        if eye.stats.roi.all_stim_only || eye.stats.covar
            ...
            if eye.stats.covar
                if ischar(eye.stim.covar)
                    eye.stim.covar = str2double(eye.stim.covar);
                end;
                covar=cell(TRIAL_COUNT,length(eye.stim.covar));
                for j= 1:TRIAL_COUNT
                    for i=1:length(eye.stim.covar)
                        act= textscan(raw{1}{IX(j)},[repmat('%*s',1,eye.stim.covar(i)-1) '%s%[...]']);
                        covar(j,i) = {act{1}{1}};
                    end;
                end;
            end;
        ...
        for j = 1:length(varlist)
            ...
            elseif strfind(varlist(j).var,'snr')
                ...
                if eye.stats.roi.all_stim_only
                    ...
                    %actual stim pic
                    act = textscan(raw{1}{IX(trlNr)},[repmat('%*s',1,eye.stim.col-1) '%s%[...]']);
                    pic = act{1}{1};
...
if eye.stats.covar
    for co =1:length(eye.stim.covar)
        cov = ['cov' num2str(co)];
